...
{
    public partial class PostListType23Page : ContentPage
    {
        ...
        public PostListType23Page()
        {
            InitializeComponent();

            Segment.ItemsSource = new List<string> { Language.can_mua, Language.can_thue };
            Segment.SetActive(0);

            this.BindingContext = viewModel = new PostListTypePageViewModel();
            this.ListView0.BindingContext = viewModelType0 = new PostListPageViewModel(2);
            this.ListView1.BindingContext = viewModelType1 = new PostListPageViewModel(3);

            ListView0.ItemTemplate = new DataTemplate(typeof(Cells.PostNoImageViewCell));
            ListView1.ItemTemplate = new DataTemplate(typeof(Cells.PostNoImageViewCell));
            Init();
        }
        ...
        public async void GoToMap_Clicked(object sender, EventArgs e)
        {
            ...
            if (index == 0)
            {
                filterModel.PostType = 2;
            }
            else
            {
                filterModel.PostType = 3;
            }
        ...
        {
            await Shell.Current.Navigation.PushAsync(new FilterPage(CurrentIndex + 2));
        }
