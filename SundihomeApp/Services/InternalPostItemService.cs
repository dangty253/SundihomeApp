...
{
    public class InternalPostItemService : IInternalPostItemService
    {
        public IMongoCollection<InternalPostItem> _postItems { get; private set; }
        public IMongoCollection<PostItemComment> _postItemComments { get; private set; }

        public InternalPostItemService()
        {
            var _mongoDbService = DependencyService.Get<IMongoDbService>();
            _postItems = _mongoDbService.GetCollection<InternalPostItem>("InternalPostItem");
            _postItemComments = _mongoDbService.GetCollection<PostItemComment>("PostItemComment");
        }

        public List<InternalPostItem> GetPostItems(int page = 1)
        {
            int limit = 10;
            int skip = (page - 1) * limit;
            return _postItems.Find(x => true).SortByDescending(x => x.CreatedDate).Skip(skip).Limit(limit).ToList();
        }
        public async Task<InternalPostItem> GetById(string Id)
        {
        ...
        public async Task RemovePostItem(string Id)
        {
            ...
            if (post.HasImage)
            {
                var response = await ApiHelper.Delete(Configuration.ApiRouter.DELETE_IMAGE + "?bucketName=sundihome/internalpostitem&files=" + string.Join(",", post.Images));
            }
